...
unsafe impl GlobalAlloc for ReportingAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        eprintln!("{}\t{}", n_bytes, delta.as_nanos());
        ptr
    }
...
impl Particle {
    fn new(world: &World) -> Particle {
        ...
        let y = world.height;
        let (vx, vy) = (0.0, rng.gen_range(-2.0..0.0));
        let (ax, ay) = (0.0, rng.gen_range(0.0..0.15));
        Particle {
            height: 4.0, width: 4.0,
            position: [x, y].into(),
            velocity: [vx, vy].into(),
            acceleration: [ax, ay].into(),
...
fn main() {
    ...
    let mut window: PistonWindow =
        WindowSettings::new("particles", [w, h])
        .exit_on_esc(true)
        .build()
